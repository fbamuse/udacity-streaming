...
gearPositionSelectStarDF.selectExpr("cast(truckId as string) as key", "cast(gearPosition as string) as value").writeStream.format("kafka").option("kafka.bootstrap.servers", "localhost:9092").option("topic", "gear-position-updates").option("checkpointLocation","/tmp/kafkacheckpoint").start().awaitTermination()