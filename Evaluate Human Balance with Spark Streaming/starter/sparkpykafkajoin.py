...
spark = SparkSession.builder.appName("STEDI-3").getOrCreate()
...
zSetEntriesEncodedStreamingDF=spark.sql("select zSetEntries[0].element as redisEvent from RedisSortedSet")
...
zSetDecodedEntriesStreamingDF1= zSetEntriesEncodedStreamingDF.withColumn("customer", unbase64(zSetEntriesEncodedStreamingDF.redisEvent).cast("string"))
...
customerStreamingDF = spark.sql("select customerName,email, birthDay  from Customer where email is not null AND birthDay is not null")
# TO-DO: Split the birth year as a separate field from the birthday
emailAndBirthDayStreamingDF = customerStreamingDF.withColumn('birthYear', split(customerStreamingDF.birthDay,"-").getItem(0))
...
steditreamDF.withColumn("value", from_json("value", customerRiskSchema)) \
    .select(col('value.*')) \
